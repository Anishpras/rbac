...
export class RBACManager {
  ...
  private checkRoleHierarchyIntegrity(removedRole: Role): void {
    try {
      ...
      if (dependentRoles.length > 0) {
        console.warn(
          `Role hierarchy integrity warning: Roles [${dependentRoles.join(", ")}] inherit from removed role ${removedRole}`,
        );
      }
    } catch (error) {
  ...
  public revoke(role: Role, resource: Resource, permission?: Permission | Permission[]): void {
    ...
    // Audit log the permission change
    console.info(
      `Revoke: Role=${role}, Resource=${resource}, Permissions=${permissions.join(",")}`,
    );
    ...
    return async (req: Request, res: Response, next: NextFunction) => {
      ...
      try {
        ...
        if (options.auditLog) {
          ...
          console.info(
            `Access attempt: User=${userId}, Roles=${JSON.stringify(roles)}, Resource=${resource}, Permission=${permission}`,
          );
        }
        ...
        RBACManager.denyAccess(
          "Access denied due to an internal error",
          res,
          req,
          next,
          options.onDenied,
